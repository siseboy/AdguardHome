...
}
{
    ...
    for (i = 1; i <= n-1; i++) {
        ...
        if ((i > 1) && (suffix in suffixes)) {
            effective_domain = parts[i-1]
            for (k = i; k <= n; k++) {
                effective_domain = effective_domain "." parts[k]
            }
            best_match = effective_domain
            break
        }
    }
    if (best_match == "") {
        if (n >= 2) {
            best_match = parts[n-1] "." parts[n]
        } else {
            best_match = domain
        }
    }
    print best_match
}' "./lite_cnacc_data.tmp" | sort | uniq))

    lite_gfwlist_data=($(awk '
BEGIN {
    while ((getline suffix < "./public_suffixes.tmp") > 0) {
        if (suffix != "" && substr(suffix,1,1) != "*") {
            suffixes[suffix] = 1
